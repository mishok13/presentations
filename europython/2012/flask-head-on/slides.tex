...
\begin{frame}
  ...
  \only<1>{
  \begin{itemize}
  \item Rule creates regexp and collects proper converters
  \item Map holds all rules and builds the string for Rule to match
  \item Converters convert the path parts into Python objects
  \end{itemize}
}
% Flask also tries to save some time matching URLs and sorts
% Rules in specific order, see the key function for Rule
\only<2>{EXAMPLES GOES HERE}
\only<3>{Another interesting detail -- Map has Rules sorted in such way
that rules that match faster will come first}
\end{frame}
